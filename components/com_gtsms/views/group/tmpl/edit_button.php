<div class="command form-inline">
	<?php if($this->isNew):?>
		...
		<div class="pull-right">
			<button type="button" class="btn btn-orange" onclick="submitform('group.cancel')">
	...
	<?php else:?>
		...
		<div class="pull-right">
			...
			<?php if($this->isTrashed  && $this->canDelete):?>
				<button type="button" class="btn btn-red" onclick="submitbuttonDelete('groups.deleteList')">
					<i class="fa fa-trash-o"></i> <?php echo JText::_('COM_GTSMS_TOOLBAR_TRASH_PERMANENTLY')?>
				</button>
			<?php elseif($this->canEditState):?>
				<button type="button" class="btn btn-red" onclick="submitbuttonDelete('groups.trash')">
