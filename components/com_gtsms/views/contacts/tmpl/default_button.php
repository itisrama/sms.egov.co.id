...
<div class="command form-inline">
	<?php if($this->canCreate):?>
		<button type="button" class="btn btn-success" onclick="Joomla.submitbutton('contact.add')">
			<i class="fa fa-plus-circle"></i> <?php echo str_replace('%s', JText::_('COM_GTSMS_CONTACT'), JText::_('COM_GTSMS_PT_NEW'))?>
		</button>
	<?php endif;?>
	...
	<div class="pull-right">
		<?php if($this->canDelete):?>
			<?php if($this->state->get('filter.published') == -2):?>
				<button type="button" class="btn btn-red" onclick="submitbuttonlist('contacts.delete')">
					<i class="fa fa-trash-o"></i> <?php echo JText::_('COM_GTSMS_TOOLBAR_TRASH_PERMANENTLY')?>
				</button>
			<?php else:?>
				<button type="button" class="btn btn-red" onclick="submitbuttonlist('contacts.trash')">
