...
class JFormFieldTableGrid extends JFormField
{
	...
	protected function getInput() {
		...
		if(!$disableAdd) {
			...
			$str[] = '<i class="fa fa-plus"></i> ' . str_replace('%s', $labeltext, JText::_('COM_GTJOBORDER_PT_NEW'));
			$str[] = '</button>';
		}
		...
		$str[] = '<h4 class="modal-title edit">'.str_replace('%s', $labeltext, JText::_('COM_GTJOBORDER_PT_EDIT')).'</h4>';
		$str[] = '<h4 class="modal-title new">'.str_replace('%s', $labeltext, JText::_('COM_GTJOBORDER_PT_NEW')).'</h4>';
		$str[] = '</div>';
		...
		$str[] = '<thead>';
		$str[] = '<tr>';
		$str[] = '<th style="text-align:center" width="80px">' . JText::_('COM_GTJOBORDER_ACTION') . '</th>';
		foreach ($fields as $field) {
			if (in_array($field->type, array('Hidden', 'AltHidden'))) {
		...
		foreach ($fields as $field) {
			...
			$str[] = '<td class="' . $field->element['name'].($field->type == 'Textarea' ? ' longtext' : null).'" style="' . implode(';', array_filter($style)) . '">';
			$str[] = '<span></span><input type="hidden"></input></td>';
		}
		...
		$str[] = '<tbody class="noItem"><tr><td style="text-align:center" colspan="' . (count($names)+1) . '">' . JText::_('COM_GTJOBORDER_NO_DATA') . '</td></tr></tbody>';
