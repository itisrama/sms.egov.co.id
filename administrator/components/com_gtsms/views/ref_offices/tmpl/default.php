...
$canOrder  = $user->authorise('core.edit.state', 'com_gtsms.ref_office');
...
');
?>
<form action="<?php echo JRoute::_('index.php?option=com_gtsms&view=ref_offices'); ?>" method="post" name="adminForm" id="adminForm">
		...
		<?php else : ?>
			<table class="table table-striped" id="contactList">
				<thead>
					<tr>
						...
						<th><div class="text-center">
							<?php echo JHtml::_('grid.sort', 'COM_GTSMS_FIELD_NAME', 'a.name', $listDirn, $listOrder); ?>
						</div></th>
						<th width="1%" class="nowrap center hidden-phone"><div class="text-center">
				...
				foreach ($this->items as $i => $item) :
					...
					$item->link	= JRoute::_('index.php?option=com_gtsms&task=ref_office.edit&id=' . $item->id);
					?>
					<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->id?>">
						...
						<td class="center">
							<div class="btn-group">
								<?php echo JHtml::_('jgrid.published', $item->published, $i, 'ref_offices.', $this->canEditState, 'cb'); ?>
								<?php
								// Create dropdown items
								$action = $archived ? 'unarchive' : 'archive';
								JHtml::_('actionsdropdown.' . $action, 'cb' . $i, 'ref_offices');
